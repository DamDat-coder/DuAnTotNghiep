...
export function mapNameToType(name?: string): string {
  ...
  if (topKeywords.some(k => lower.includes(k))) return "top";
  ...
  if (bottomKeywords.some(k => lower.includes(k))) return "bottom";
  ...
  const outerKeywords = ["Áo khoác gió",
    ...
    "Áo khoác bomber nữ",
    "Áo sweater nữ"];
  if (outerKeywords.some(k => lower.includes(k))) return "outer";

  return "other";
}

// =================== 2. Nhóm vào group chính ===================
export function mapTypeToGroup(type: string): "top" | "bottom" | "outer" | "other" {
  if (type === "top" || type === "bottom" || type === "outer") return type;
  return "other";
}
...
`;
  ...
  const raw = (res.content as string).replace(/```json|```/g, "").trim();
...
function sanitizeAIResult(aiResult: any, productsWithType: any[]) {
  ...
  const pickUniqueByType = (type: string) => {
  ...
  const buildOutfit = (ids: string[], requiredTypes: string[], limit: number) => {
    const result: string[] = filterValid(ids).filter(id => !usedIds.has(id));
