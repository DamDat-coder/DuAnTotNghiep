        ...
        if (sort) {
            ...
            if (!sortOptions[sort]) {
                res
                    ...
                    .json({
                    status: "error",
                    message: "Tùy chọn sắp xếp không hợp lệ. Chỉ hỗ trợ: newest, best-seller, name-asc, name-desc",
                });
                return;
            }
...
const getAllProducts = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (is_active !== undefined) {
            if (is_active !== "true" && is_active !== "false") {
                res
                    ...
                    .json({
                    status: "error",
                    message: "Invalid is_active value, must be true or false",
                });
                return;
            }
        ...
        if (id_cate) {
            ...
            query["category._id"] = {
                $in: categoryIds.map((id) => new mongoose_1.default.Types.ObjectId(id)),
            };
        }
        ...
        if (priceRange) {
            ...
            if (!filter) {
                res
                    ...
                    .json({
                    status: "error",
                    message: `Invalid price range: ${priceRange}`,
                });
                return;
            }
            priceMatch = filter;
        ...
        if (sort === "price-asc" || sort === "price-desc") {
            pipeline.push({
                $sort: { minDiscountedPrice: sort === "price-asc" ? 1 : -1 },
            });
        }
        else {
            const sortOptions = {
...
const getProductById = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const product = yield productModel_1.default.findById(req.params.id).lean();
        if (!product) {
            res
                .status(404)
...
const getProductBySlug = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const { exact = "true" } = req.query; // Mặc định exact=true
        ...
        if (exact === "true") {
            // Tìm chính xác slug
            products = yield productModel_1.default.findOne({ slug }).lean();
            if (!products) {
                res
                    .status(404)
                    .json({ status: "error", message: "Sản phẩm không tồn tại" });
            ...
            // Định dạng kết quả cho một sản phẩm
            const result = Object.assign(Object.assign({}, products), { category: {
                    _id: ((_a = products.category) === null || _a === void 0 ? void 0 : _a._id) || null,
                    name: ((_b = products.category) === null || _b === void 0 ? void 0 : _b.name) || "Không rõ",
                } });
            res.status(200).json({ status: "success", data: result });
        }
        else {
            // Tìm gần đúng với regex
            ...
            // Định dạng kết quả cho danh sách sản phẩm
            ...
            res
                .status(200)
                .json({ status: "success", data: result, total: result.length });
        }
    }
    catch (error) {
        res.status(500).json({
