...
export default function PhoneVerifyModal({
  ...
  const handleSendCode = async () => {
    ...
    if (!accepted) return setError("Bạn cần đồng ý điều khoản.");
    setLoading(true);
    try {
      await sendOtp(phone);
      setStep("otp");
        ...
        {step === "input" && (
          <>
            ...
            <input
              ...
              className="w-[440px] h-[40px] border rounded-[8px] mb-2 px-3"
            />
            <span className="text-[12px] text-[#999999]">
              *Vui lòng nhập số điện thoại Việt Nam (bắt đầu bằng 0, đủ 10-11
              số)
            </span>
