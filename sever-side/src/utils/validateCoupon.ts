...
export const validateCoupon = async ({
    ...
    if (hasProductCondition || hasCategoryCondition) {
        ...
        for (let i = 0; i < productIds.length; i++) {
            ...
            if (hasProductCondition) {
                productMatch = coupon.applicableProducts!.some(
                    (p) => p.toString() === productId.toString()
                );
            }

            if (hasCategoryCondition) {
                const expandedCouponCategoryIds: string[] = [];
                for (const cat of coupon.applicableCategories!) {
                    const childIds = await getAllChildCategoryIds(cat.toString());
    ...
    if (coupon.minOrderAmount && totalAmount < coupon.minOrderAmount) {
        throw new Error(
            `Đơn hàng phải có giá trị tối thiểu ${coupon.minOrderAmount.toLocaleString()}đ để áp dụng mã`
        );
    }

    if (coupon.maxOrderAmount && totalAmount > coupon.maxOrderAmount) {
        throw new Error(
            `Đơn hàng vượt quá giá trị tối đa ${coupon.maxOrderAmount.toLocaleString()}đ được phép dùng mã này`
        );
    }
