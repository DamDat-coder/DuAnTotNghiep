...
export default function ProductBody({ products, onEdit, onDelete }) {
  const [switchStates, setSwitchStates] = useState(() =>
    products.reduce((acc, p) => ({ ...acc, [p._id]: true }), {})
  );
  ...
  return (
    <>
      {products.map(product => (
        <tr key={product._id} className="border-b hover:bg-[#F9FAFB] transition-colors duration-150 group">
          <td className="px-4 py-4 min-w-[270px] max-w-[350px] align-middle">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 min-w-[40px] min-h-[40px] rounded-lg overflow-hidden bg-[#f3f3f3] flex items-center justify-center">
                <Image
                  src={product.image && product.image[0] ? `/uploads/${product.image[0]}` : "/no-image.png"}
                  width={40}
                  height={40}
          ...
          <td className="px-4 py-4 min-w-[70px] align-middle">{"--"}</td>
          ...
          <td className="px-4 py-4 min-w-[100px] align-middle">
            <button
              className={`w-10 h-6 rounded-full transition relative ${
                switchStates[product._id] ? "bg-[#2563EB]" : "bg-gray-300"
              }`}
            ...
            >
              <span
                className={`absolute left-0 top-0 transition-all duration-200 w-6 h-6 bg-white rounded-full shadow ${
                  switchStates[product._id] ? "translate-x-4" : "translate-x-0"
                }`}
              />
            </button>
          </td>
          ...
          <th className="w-[64px] px-4 py-0 rounded-tr-[12px] rounded-br-[12px] align-middle relative">
            <div className="flex items-center justify-end h-[64px]">
              <button
                ...
                onClick={e => {
                  e.stopPropagation();
                  setActionDropdownId(actionDropdownId === product._id ? null : product._id);
                }}
              >
                <Image
